...
function invia(){
  ...
  const posizione = document.getElementById('location');
  const lavoro = document.getElementById('job');
  let newElement = []

  let counter = 0;

  for (let i = 0; i < jobs.length; i++) {
    if (jobs[i].location.toLowerCase().includes(posizione.value.toLowerCase()) && jobs[i].title.toLowerCase().includes(lavoro.value.toLowerCase())) {

      newElement[i] = document.createElement("li");
      newElement[i].innerText = jobs[i].title + " \n- " + jobs[i].location;
      lista.appendChild(newElement[i]);
      counter++;
    }
    
  }
  console.log("Totale posizioni trovate: ", counter);
}
